...
import { findArtifacts, copyArtifactToOutput, getSearchDirectories, } from "./artifacts.js";
...
program
    ...
    .requiredOption("--tauri-project <path>", "Path to Tauri project directory")
    .requiredOption("--output-dir <path>", "Output directory for latest.json and artifacts")
    .requiredOption("--base-url <url>", "Base URL for artifact downloads (supports {version} placeholder)")
    .option("--notes <string>", "Release notes")
    .option("--allow-overwrite-platforms", "Allow overwriting existing platform entries in latest.json")
    .action(async (opts) => {
    const options = {
    ...
    try {
        ...
        const searchDirectories = await getSearchDirectories(options.tauriProject);
        ...
        if (artifacts.length === 0) {
            console.error("No artifacts found. Make sure you have built the Tauri app with updater artifacts enabled.");
            process.exit(1);
        }
        ...
        const latestJson = await generateLatestJson(artifacts, version, options.baseUrl, options.outputDir, options.notes, options.allowOverwritePlatforms);
